...
public class PlaybackXMLEmbedded {
	private static final String COPYBOOK_PAYLOAD_XPATH = "//*[local-name() = 'ProcessMessageResult']";
	protected static Logger log   = Logger.getLogger(PlaybackXMLEmbedded.class.getName());
	
	private Document doc;
	private Node copybookPayloadNode;
	
	public PlaybackXMLEmbedded(String xml) {
		try {
	...
	public String extract() {
		try {
			...
			if (obj instanceof Element) {
				...
				copybookPayloadNode = node.getFirstChild();
				if(copybookPayloadNode instanceof Element){
					return XMLUtils.toXML((Element)copybookPayloadNode);
					
				} else if (copybookPayloadNode instanceof DeferredNode){
					return copybookPayloadNode.getTextContent();
				}
			} else {
	...
	public static void main(String[] args) throws Exception {
		String xml = "<soapenv:Envelope xmlns=\"http://will.power\" xmlns:soapenv=\"http://www.w3.org/2003/05/soap-envelope\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n" + 
				...
				"    </soapenv:Body>\n" + 
				"</soapenv:Envelope>";
		PlaybackXMLEmbedded p = new PlaybackXMLEmbedded(xml);
		p.extract();
		p.inject("HSBC29");
