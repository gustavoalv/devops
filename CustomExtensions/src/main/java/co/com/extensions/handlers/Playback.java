...
public class Playback {
	...
	public Playback(String xml) {
		try {
			document = XMLUtils.newXMLDocument(new InputSource(new StringReader(xml)));
		} catch (IOException e) {
	...
	public void inject(String copybook) {
		if (copybookPayloadNode != null) {
			...
			parentNode.appendChild(document.createTextNode(copybook));
		}
	}
	
	public String toString() {
		try {
			return XMLUtils.toXML(document.getDocumentElement());
		} catch (Exception e) {
			return "";
		}
	}
