 ...
 *
 */
public class AutraTCPDelimiter implements TCPDelimiter{
    private static final Logger log = Logger.getLogger(AutraTCPDelimiter.class);
    ...
    public boolean locateRequest(List<Byte> bytes) {
        ...
        if (bytes.size() > 2) {
				...
				log.info("content.substring(0, 2): " + content.substring(0, 2)); 
                ...
                String delimitador = Converter.convertByteToHex(sizeBytes);
                log.info("Delimitador: " + delimitador);
                
                log.info("content.contains(1140401D60) " + content.contains("1140401D60"));
                log.info("content.contains(114040) " + content.contains("114040"));
